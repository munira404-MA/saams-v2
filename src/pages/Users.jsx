...
const emptyForm = { full_name: '', username: '', password: '', role: 'nursery', nursery: '', active: true };
...
export default function Users({ lang }) {
  ...
  function startEdit(user) {
    ...
    setForm({ ...user, password: user.password || '' });
    setMessage('');
    setOpen(true);
  }
  ...
  function submit(e) {
    ...
    const payload = { ...form, username, nursery: form.role === 'nursery' ? form.nursery : '' };
    if (editingId) {
      ...
      <section className="users-table-card">
        ...
        <div className="users-table-wrap">
          <table className="users-table">
            <thead><tr><th>{ar ? 'الاسم' : 'Name'}</th><th>{ar ? 'اسم المستخدم' : 'Username'}</th><th>{ar ? 'نوع الحساب' : 'Account type'}</th><th>{ar ? 'الحضانة' : 'Nursery'}</th><th>{ar ? 'الحالة' : 'Status'}</th><th>{ar ? 'الإجراءات' : 'Actions'}</th></tr></thead>
      ...
      {open && <div className="modal-backdrop" onMouseDown={() => setOpen(false)}>
        <form className="user-modal" onMouseDown={(e) => e.stopPropagation()} onSubmit={submit}>
          ...
          <div className="user-form-grid">
            ...
            <label><span>{ar ? 'نوع الحساب' : 'Account type'}</span><select value={form.role} onChange={(e) => setForm({ ...form, role: e.target.value, nursery: '' })}><option value="nursery">{ar ? 'حساب حضانة' : 'Nursery account'}</option><option value="admin">{ar ? 'موظف إدارة' : 'Administration employee'}</option><option value="super_admin">{ar ? 'مدير النظام' : 'System administrator'}</option></select></label>
